...
import { Ether, NativeToken } from '../../entities'
...
describe('CurrencyAmount', () => {
  ...
  describe('constructor', () => {
    it('works', () => {
      const token = new NativeToken(1, ADDRESS_ONE, 18)
  ...
  it('token amount can be max uint256', () => {
    const amount = CurrencyAmount.fromRawAmount(new NativeToken(1, ADDRESS_ONE, 18), MaxUint256)
    expect(amount.quotient).toEqual(MaxUint256)
  })
  it('token amount cannot exceed max uint256', () => {
    expect(() =>
      CurrencyAmount.fromRawAmount(new NativeToken(1, ADDRESS_ONE, 18), JSBI.add(MaxUint256, JSBI.BigInt(1)))
    ).toThrow('AMOUNT')
  })
  it('token amount quotient cannot exceed max uint256', () => {
    expect(() =>
      CurrencyAmount.fromFractionalAmount(
        new NativeToken(1, ADDRESS_ONE, 18),
  ...
  describe('#toFixed', () => {
    it('throws for decimals > currency.decimals', () => {
      const token = new NativeToken(1, ADDRESS_ONE, 0)
