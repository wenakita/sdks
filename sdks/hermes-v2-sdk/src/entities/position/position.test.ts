...
import { NativeToken, Percent } from 'maia-core-sdk'

import { FeeAmount, TICK_SPACINGS } from '../../constants/constants'
...
describe('Position', () => {
  const USDC = new NativeToken(1, '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', 6, 'USDC', 'USD Coin')
  const DAI = new NativeToken(1, '0x6B175474E89094C44Da98b954EedeAC495271d0F', 18, 'DAI', 'DAI Stablecoin')
  ...
  it('can be constructed around 0 tick', () => {
    ...
    expect(position.liquidity instanceof JSBI).toEqual(true)
  })
  ...
  describe('#mintAmountsWithSlippage', () => {
    ...
    describe('5% slippage tolerance', () => {
      ...
      it('is correct for pool at max price', () => {
        ...
        const { amount0, amount1 } = position.burnAmountsWithSlippage(slippageTolerance) ?? { undefined }
        expect(amount0?.toString()).toEqual('0')
  ...
  describe('#mintAmounts', () => {
    it('is correct for price above', () => {
      const { amount0, amount1 } = new Position({
        ...
        tickLower: nearestUsableTick(POOL_TICK_CURRENT, TICK_SPACING) + TICK_SPACING,
        tickUpper: nearestUsableTick(POOL_TICK_CURRENT, TICK_SPACING) + TICK_SPACING * 2,
      }).mintAmounts ?? { amount0: JSBI.BigInt(0), amount1: JSBI.BigInt(0) }
