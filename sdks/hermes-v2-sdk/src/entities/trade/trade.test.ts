...
import { FeeAmount, TICK_SPACINGS, TradeType } from '../../constants/constants'
...
describe('Trade', () => {
  ...
  describe('#fromRoutes', () => {
    it('can be constructed with ETHER as input with multiple routes', async () => {
      const trade = await Trade.fromRoutes<Pool, Ether, NativeToken, TradeType.EXACT_INPUT>(
        [
          {
    ...
    it('can be constructed with ETHER as input for exact output with multiple routes', async () => {
      const trade = await Trade.fromRoutes<Pool, Ether, NativeToken, TradeType.EXACT_OUTPUT>(
        [
          {
    ...
    it('can be constructed with ETHER as output with multiple routes', async () => {
      const trade = await Trade.fromRoutes<Pool, NativeToken, Ether, TradeType.EXACT_OUTPUT>(
        [
          {
    ...
    it('can be constructed with ETHER as output for exact input with multiple routes', async () => {
      const trade = await Trade.fromRoutes<Pool, NativeToken, Ether, TradeType.EXACT_INPUT>(
        [
          {
            amount: CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(3000)),
            route: new Route([pool_weth_0], token0, ETHER),
    ...
    it('throws if pools are re-used between routes', async () => {
      await expect(
        Trade.fromRoutes<Pool, NativeToken, Ether, TradeType.EXACT_INPUT>(
          [
            {
  ...
  describe('#maximumAmountIn', () => {
    describe('tradeType = EXACT_INPUT', () => {
      let exactIn: Trade<Pool, NativeToken, NativeToken, TradeType.EXACT_INPUT>
      beforeEach(async () => {
    ...
    describe('tradeType = EXACT_OUTPUT', () => {
      let exactOut: Trade<Pool, NativeToken, NativeToken, TradeType.EXACT_OUTPUT>
      beforeEach(async () => {
