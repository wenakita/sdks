...
>(a: Trade<TPool, TInput, TOutput, TTradeType>, b: Trade<TPool, TInput, TOutput, TTradeType>) {
...
> {
   ...
   */
  public get route(): Route<TPool, TInput, TOutput> {
   ...
   */
  public static async exactIn<TPool extends IPool, TInput extends Currency, TOutput extends Currency>(
    route: Route<TPool, TInput, TOutput>,
    amountIn: CurrencyAmount<TInput>
  ): Promise<Trade<TPool, TInput, TOutput, TradeType.EXACT_INPUT>> {
   ...
   */
  public static async exactOut<TPool extends IPool, TInput extends Currency, TOutput extends Currency>(
    route: Route<TPool, TInput, TOutput>,
    amountOut: CurrencyAmount<TOutput>
  ): Promise<Trade<TPool, TInput, TOutput, TradeType.EXACT_OUTPUT>> {
  ...
  ): Promise<Trade<TPool, TInput, TOutput, TTradeType>> {
    const amounts: CurrencyAmount<NativeToken>[] = new Array(route.tokenPath.length)
  ...
  >(constructorArguments: {
    ...
    outputAmount: CurrencyAmount<TOutput>
    tradeType: TTradeType
  }): Trade<TPool, TInput, TOutput, TTradeType> {
    return new Trade({
      ...constructorArguments,
      routes: [
        {
  ...
  private constructor({
    ...
    for (const { route } of routes) {
      for (const pool of route.pools as IPool[]) {
   ...
   * @returns The exact in trade
   */
  public static async bestTradeExactIn<TPool extends IPool, TInput extends Currency, TOutput extends Currency>(
    pools: TPool[],
    ...
    bestTrades: Trade<TPool, TInput, TOutput, TradeType.EXACT_INPUT>[] = []
  ): Promise<Trade<TPool, TInput, TOutput, TradeType.EXACT_INPUT>[]> {
   ...
   * @returns The exact out trade
   */
  public static async bestTradeExactOut<TPool extends IPool, TInput extends Currency, TOutput extends Currency>(
    pools: TPool[],
    currencyIn: TInput,
    ...
    bestTrades: Trade<TPool, TInput, TOutput, TradeType.EXACT_OUTPUT>[] = []
  ): Promise<Trade<TPool, TInput, TOutput, TradeType.EXACT_OUTPUT>[]> {
    invariant(pools.length > 0, 'POOLS')
    invariant(maxHops > 0, 'MAX_HOPS')
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      let amountIn: CurrencyAmount<NativeToken>
      try {
