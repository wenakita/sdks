...
*/
function _tokenInForExactBPTOut(amount: OldBigNumber, poolPairData: StablePoolPairData): OldBigNumber {
  // The formula below returns some dust (due to rounding errors) but when
  // we input zero the output should be zero
  if (amount.isZero()) return amount
  const { amp, allBalances, tokenIndexIn, tokenIndexOut, swapFee } = poolPairData
  ...
  // return amountIn
  return amountInAfterFee.div(ONE.minus(tokenBalancePercentageExcess.times(swapFee.toString()).div(EONE.toString())))
}
